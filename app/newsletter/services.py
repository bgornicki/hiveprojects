...
class NewsletterSender:
    ...
    def get_favorite_project_events(user):
        newsletter_cache, _ = NewsletterCache.objects.get_or_create(user=user)
        favorite_projects = NewsletterSender.get_user_favorite_projects(user)
        timeline_events = TimelineEvent.objects.filter(
            project__in=favorite_projects,
            date__gte=datetime.now() - timedelta(days=NewsletterSender.NEWSLETTER_FREQUENCY_IN_DAYS))
        return timeline_events

    @staticmethod
    def get_latest_projects():
        return Project.objects.all().order_by('-id')[:NewsletterSender.AMOUNT_OF_LATEST_PROJECTS_IN_NEWSLETTER]

    @staticmethod
    def send_newsletter(user):
        favorite_project_events = NewsletterSender.get_favorite_project_events(user)
        if not favorite_project_events:
            return False

        latest_projects = NewsletterSender.get_latest_projects()

        unsubscribe_link = NewsletterSender.get_unsubscribe_link(user)
